...
export const Navbar = () => {
    ...
    return (
        <header>
            {/* Nabar */}
            <nav className="max-w-7xl bg-white shadow-2xl mx-auto flex justify-between p-5 rounded-2xl ">
                {/* Logo */}
                ...
                <ul className="flex gap-4 ">
                    {
                        tabs.map((items) => (
                            <>
                                <NavLink key={items.id} to={items.path}>{items.lable}</NavLink>
                            </>
                        ))
                    }
